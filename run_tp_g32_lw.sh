...
# 拼接完整的文件名
SPLIT="Lean-Workbook/lean_statement_part_${SPLIT_NUM}"
...
for i in $(seq 0 $((WORLD_SIZE-1))); do
    ...
    nohup python infer/infer.py \
      ...
      --use_accel \
      --index "$i" \
      --world_size "$WORLD_SIZE" > "$LOG_DIR/g32_worker_${SPLIT_NUM}_${i}_${WORLD_SIZE}_tp${TP_VALUE}.log" 2>&1 &
...
echo "Use 'tail -f $LOG_DIR/g32_worker_${SPLIT_NUM}_*_${WORLD_SIZE}_tp${TP_VALUE}.log' to monitor logs"
